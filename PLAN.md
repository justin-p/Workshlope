...
| Field | Value |
| ------ | ------ |
| **Last synced** | **2026-05-07** — Lesson follow-up stack is now fully landed on **`main`**: [#30](https://github.com/justin-p/testing/pull/30) merged directly and remaining lesson branches were merged into `main` in order (L2-L5), followed by plan/rules cleanup (`PLAN.md` rename + AGENTS policy consolidation). In-flight local now extends instructor lesson-repo UX with manifest-audit metadata surfaced in repo list API/UI, a per-repo lesson/parts preview endpoint consumed by the sync card, installation-level direct settings links, backend installation-scoped repo list filtering, an app install CTA URL from installation metadata, health-focused repo filtering, server-backed synced-repo text search (`q`) wired to a dashboard filter input, functionality-first workshop-run readiness gates, workshop session detail lesson-source metadata for degraded repo health and missing lesson-part content availability, automatic websocket reconnect attempts when `part_generation_stale` is received, a serial Playwright end-to-end instructor-led flow gate covering trainee unblock, live controls, and room closeout, defensive workshop session detail fallbacks when lesson or lesson-repo rows are unavailable, UI guardrails that disable live instructor/timer controls when lesson content is unavailable, and a `Retry lesson check` recovery action on the workshop page for degraded content.
...
| Lesson source sync can keep workshop content available | lesson repo sync API/UI + manifest pipeline | 🟨 | Session detail now surfaces repo health/last sync plus `lesson_content_available` + `lesson_content_issue`; workshop UI shows degraded-source fallback and blocking no-parts banner. Continue treating missing lessons/parts after sync as blocking. |
