...
| **Last synced** | **2026-05-11** — **`feat/dashboard/multi-lesson-repo-session`:** merged latest **`main`** (includes roster trainee removal + picker table alignment **[#56](https://github.com/justin-p/testing/pull/56)** and unified lesson-repo owner/name **`datalist`** **[#57](https://github.com/justin-p/testing/pull/57)**). This slice: multi-lesson **Choose lesson**, per-lesson **Start workshop** + scoped feedback, Playwright in [`dashboard-routing.spec.ts`](frontend/tests/dashboard-routing.spec.ts). |
