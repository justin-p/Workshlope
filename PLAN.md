...
| **Last synced** | **2026-05-12** — **`main`** includes **[#77](https://github.com/justin-p/testing/pull/77)** (drift ack, revoke, `active_badge_grants`, e2e bump) and merged **[#78](https://github.com/justin-p/testing/pull/78)** (badge hub, manifest v2 `badges[]`, org grants, global leaderboard, migrations **`g1h2i3j4k5l6`** + merge head **`a6357a27fcd7`**). **Blocking:** **§1 Workshop** only. Prior: **[#76](https://github.com/justin-p/testing/pull/76)**. |
| **Branch** | **`main`** |
| **PR** | **[#78](https://github.com/justin-p/testing/pull/78)** merged |
...
| 2026-05-12 | Instructor Workshop Page | Controls (start/stop, pacing, etc.) should be sticky and follow the instructor as they scroll or resize the page. | Functionality-first focus; polish deferred until core flow validated end-to-end. |
| 2026-05-12 | Trainee Roster / Invites | Separate "Invite Trainees" area from the current active/present trainees list for instructor clarity. | Not blocking workshop run flow; defer until initial end-to-end paths hardened. |
...
| 2026-05-12 | Workshop session (realtime) | **Mid-session WebSocket drop** UX after `session.connected`: clear disconnected state, **toast** + **retry / resilient ws-ticket refresh** (backoff), aligned with spec "Reconnect: toast + resilient ws-ticket refresh" — avoid silent dead sockets while UI still reads "connected". | Reconnect hardening is polish once baseline live session behavior is stable. |
| 2026-05-12 | Workshop session (browser chrome) | Dynamic **document title** (tab strip): lesson title and optional session status (e.g. Paused) instead of static "Workshop session" meta. | Reduces wrong-tab mistakes during long runs; non-blocking vs execution correctness. |
| 2026-05-12 | Trainee session header | **Instructor attribution** line (single allowed name/string, not a roster), per trainee session header in [UI / UX specification](#ui--ux-specification). | Generic copy exists today; named attribution deferred until DTO/copy rules confirmed non-sensitive. |
| 2026-05-12 | Instructor cockpit (responsive) | **Narrow-viewport layout** per locked spec: stack with **horizontally scroll chips or collapsible drawer** for roster rail vs single long column that obscures lesson on phones/tablets. | Related to sticky controls row; defer full responsive pass until core cockpit behavior signed off. |
| 2026-05-12 | Instructor live facilitation | **focus order** through destructive actions, and confirm-dialog tuning to reduce mis-clicks during live delivery. | Ergonomics layer on top of functional controls; defer until controls finalized. |
| 2026-05-12 | Copy / IA | Single **microcopy pass** for consistent use of "Workshop" / "Session" / "Lesson part" across [`workshop.$sessionId.tsx`](frontend/src/routes/_layout/workshop.$sessionId.tsx) and linked dashboard entry points. | Cosmetic consistency; defer until strings stable after functional churn. |
| 2026-05-12 | Dashboards | **Stub rails vs content matrix**: replace or clarify [`DashboardStubRails.tsx`](frontend/src/components/dashboard/DashboardStubRails.tsx) placeholders so they do not read like broken features relative to [Dashboard content matrix](#dashboard-content-matrix). | Matrix partially implemented via shared session list; polish when persona home pages are in scope. |
| 2026-05-12 | Instructor cockpit (presentation surface) | **"Open workshop content" (new tab):** Add a clear **Open workshop content** action that opens **live** lesson/session Markdown in a **new browser tab** for classroom or Teams/Zoom screen-share without showing cockpit chrome beside it. **Minimum:** launch is always available; primary cockpit **does not embed** lesson content next to instructor tools. **Presentation tab:** defaults to the **trainee-clean** view (lesson/session only); instructor controls (start/pause, pacing, roster, invites, debug, etc.) stay **hidden until** an explicit **toggle or hotkey** so shared screens do not leak instructor-only UI. | Larger routing/view split plus safe instructor-as-reader semantics; defer until core live workshop path is stable end-to-end. |
...
| Pre-create session wizard (from lesson) | [`frontend/src/routes/_layout/workshop.new.tsx`](frontend/src/routes/_layout/workshop.new.tsx) |
...
- P0 sweep validation (Slice C): `tests/workshop.spec.ts` + `tests/dashboard-routing.spec.ts` + `tests/login.spec.ts` green (`45 passed`), and backend `tests/api/routes/test_workshop_sessions.py` green in compose backend runtime (`121 passed`).
