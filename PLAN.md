...
| 2026-05-12 | Instructor live facilitation | **Keyboard shortcuts** (prev/next part, pause/resume), **focus order** through destructive actions, and confirm-dialog tuning to reduce mis-clicks during live delivery. | Ergonomics layer on top of functional controls; defer until controls finalized. |
...
- **Instructor cockpit — "Open workshop content" UX:** Instead of embedding the current lesson/session content alongside the instructor controls and roster tools, the cockpit should present a clear "Open workshop content" action (button or link) that **opens the live lesson/session content in a new browser tab or window**. This enables instructors to easily share or screen-cast the workshop content tab (for example, in a classroom or Teams/Zoom call) without exposing cockpit controls.
  - In the newly opened "workshop content" tab/view, **instructor controls (start/pause, pacing, add/invite, debugging tools, etc.) should be hidden by default** for security/clarity.
    - Controls may be revealed only if the instructor **explicitly toggles them on** (e.g., via a discrete toggle or hotkey).
    - The default presentation is "clean", showing only the lesson/session view as trainees see it.
  - Rationale: Clean separation of instructor cockpit tools from the presentation surface used for sharing; prevents accidental leakage of instructor-only UI, especially in shared screen/streaming contexts.
  - Minimum: Always allow instructor to launch live session content into a new tab; cockpit never embeds lesson content directly beside controls after this change.
